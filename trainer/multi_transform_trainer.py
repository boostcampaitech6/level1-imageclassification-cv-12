...
class Multi_TransformTrainer:
    ...
    def train(self, args):
        ...
        transform_module2 = getattr(import_module("dataset"), args.augmentation2)  # default: BaseAugmentation
        transform2 = transform_module2(
            resize=args.resize,
            mean=dataset.mean,
            std=dataset.std,
        )
        dataset.set_transform(transform, transform2)
        ...
        if args.criterion == "focal":
            m_criterion = create_criterion(args.criterion, weight=m_cls_weight)
            g_criterion = create_criterion(args.criterion, weight=g_cls_weight)
            a_criterion = create_criterion(args.criterion, weight=a_cls_weight)
        else:
        ...
        for epoch in range(args.epochs):
            ...
            for idx, train_batch in enumerate(train_loader):
                inputs_mg, inputs_a, labels = train_batch
                inputs_mg = inputs_mg.to(device)
            ...
            with torch.no_grad():
                ...
                for val_batch in val_loader:
                    inputs_mg, inputs_a, labels = val_batch
                    inputs_mg = inputs_mg.to(device)
                    inputs_a = inputs_a.to(device)
                    labels = labels.to(device)
                    mask_label, gender_label, age_label = dataset.decode_multi_class(labels)
                    ...
                    if figure is None:
                        inputs_np = torch.clone(inputs_a).detach().cpu().permute(0, 2, 3, 1).numpy()
                ...
                print(
                    f"[Val] acc : {val_acc:4.2%}, loss: {val_loss:4.2}, F1 score: {val_f1:2.4} || "
                    f"best acc : {best_val_acc:4.2%}, best loss: {best_val_loss:4.2}, best f1: {best_val_f1:2.4}"
                )
