...
class WBEmulator:
    def __init__(self):
        ...
        self.features = np.load(
            "/data/ephemeral/home/level1-imageclassification-cv-12/WBAugmenter/features.npy"
        )
        # mapping functions to emulate WB effects
        self.mappingFuncs = np.load(
            "/data/ephemeral/home/level1-imageclassification-cv-12/WBAugmenter/mappingFuncs.npy"
        )
        # weight matrix for histogram encoding
        self.encoderWeights = np.load(
            "/data/ephemeral/home/level1-imageclassification-cv-12/WBAugmenter/encoderWeights.npy"
        )
        # bias vector for histogram encoding
        self.encoderBias = np.load(
            "/data/ephemeral/home/level1-imageclassification-cv-12/WBAugmenter/encoderBias.npy"
        )
    ...
    def encode(self, hist):
        ...
        feature = np.dot(hist_reshaped - self.encoderBias.transpose(), self.encoderWeights)
        return feature
    ...
    def generateWbsRGB(self, I, outNum=10):
        ...
        D_sq = (
            np.einsum("ij, ij ->i", self.features, self.features)[:, None]
            + np.einsum("ij, ij ->i", feature, feature)
            - 2 * self.features.dot(feature.T)
        )
        ...
        for i in range(len(inds)):  # for each of the retried training examples,
            ...
            mf = sum(
                np.reshape(np.matlib.repmat(weightsH, 1, 27), (self.K, 1, 9, 3))
    ...
    def single_image_processing(self, in_img, out_dir="../results", outNum=10, write_original=1):
        ...
        for i in range(outNum):  # save images
            ...
            outImg.save(out_dir + "/" + os.path.basename(filename) + wb_pf[i] + file_extension)  # save it
            if write_original == 1:
    ...
    def trainingGT_processing(self, in_dir, out_dir, gt_dir, out_gt_dir, gt_ext, outNum=10, write_original=1):
        ...
        for in_img, gtfile in zip(imgfiles, gtfiles):
            ...
            for i in range(outNum):
                ...
                outImg.save(out_dir + "/" + os.path.basename(filename) + wb_pf[i] + file_extension)  # save it
                shutil.copyfile(
                    gtfile,  # copy corresponding gt file
                ...
                if write_original == 1:  # if write_original flag is true
                    ...
                    shutil.copyfile(gtfile, os.path.join(out_gt_dir, gtbasename + "_original" + gt_extension))
