    ...
    def __init__(self, resize, mean, std, **args):
        self.transform = A.Compose(
            [
                ...
                A.ColorJitter(),
                A.GaussNoise(var_limit=(1000, 1600), p=0.3),
                A.CoarseDropout(
                    max_holes=20, max_height=25, max_width=25, min_holes=None, min_height=5, min_width=5
                ),
...
class AgeLabels(int, Enum):
    ...
    @classmethod
    def from_number2(cls, value: str) -> int:
        """숫자로부터 해당하는 나이 라벨을 찾아 반환하는 클래스 메서드"""
        try:
            value = int(value)
        except Exception:
...
class MultiTransformMaskSplitByProfileDataset(MaskBaseDataset):
    """
    train / val 나누는 기준을 이미지에 대해서 random 이 아닌 사람(profile)을 기준으로 나눕니다.
    구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다.
    이후 `split_dataset` 에서 index 에 맞게 Subset 으로 dataset 을 분기합니다.
...
class BalancedDataset2(MaskSplitByProfileDataset):
    """
    MaskSplitByProfileDataset 처럼 사람을 기준으로 train/val을 나누고 있습니다.
    갯수가 적은 클래스의 사진들을 단순 중복 입력하여 클래스 간 불균형을 완화하려합니다.
    """
        ...
        for phase, indices in split_profiles.items():
            ...
            for _idx in indices:
                ...
                for file_name in os.listdir(img_folder):
                    ...
                    gender_label = GenderLabels.from_str(gender)
                    # 남자 0, 여자 1
                    age_label = AgeLabels.from_number2(age)
                    # ~21 이면 0, 22~54 이면 1, 55이상이면 2
...
class MultiTransformBalancedDataset2(BalancedDataset2):
    """
    MaskSplitByProfileDataset 처럼 사람을 기준으로 train/val을 나누고 있습니다.
    갯수가 적은 클래스의 사진들을 단순 중복 입력하여 클래스 간 불균형을 완화하려합니다.
    """
